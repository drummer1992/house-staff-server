...
const addProducts = async (productsIds: string[], user?: User): Promise<void> => {
  ...
  const notFoundProductsIds = productsIds.filter(id => !products.find((p: { id: string }) => p.id === id))
  ...
  validationAssert(!alreadyWishlisted.length,
    'Some products are already in the wishlist: ' + alreadyWishlisted.map((p: { productId: string }) => p.productId).join(', '))
